...
class FusionTransformer(nn.Module):
    ...
    def __init__(self, width: int,
        ...
        if fusion == "concat":
            ...
            self.resblocks_0 = ResidualCrossAttentionBlock(width, n_heads, add_bias_kv=add_bias_kv,
                                            dropout=dropout, batch_first=batch_first)

            self.resblocks_1 = ResidualCrossAttentionBlock(width, n_heads, add_bias_kv=add_bias_kv,
                                            dropout=dropout, batch_first=batch_first)
        ...
        # print(f'self.resbocks: {self.resblocks}')
        self.initialize()
    ...
    def initialize(self):
        ...
        if self.fusion == "concat":
            ...
            # for block in self.resblocks_0:
            nn.init.normal_(self.resblocks_0.attn.in_proj_weight, std=attn_std)
            nn.init.normal_(self.resblocks_0.attn.out_proj.weight, std=proj_std)
            nn.init.normal_(self.resblocks_0.mlp.c_fc.weight, std=fc_std)
            nn.init.normal_(self.resblocks_0.mlp.c_proj.weight, std=proj_std)

            # for block in self.resblocks_1:
            nn.init.normal_(self.resblocks_1.attn.in_proj_weight, std=attn_std)
            nn.init.normal_(self.resblocks_1.attn.out_proj.weight, std=proj_std)
            nn.init.normal_(self.resblocks_1.mlp.c_fc.weight, std=fc_std)
            nn.init.normal_(self.resblocks_1.mlp.c_proj.weight, std=proj_std)
    ...
    def forward(self, x: List[torch.Tensor], key_padding_mask: List[torch.Tensor] = None):
        ...
        # Cross-attention + concatenate over tokens
            ...
            x1, x2 = x
            x = torch.cat([self.resblocks_0(x1, x2, key_padding_mask),
                           self.resblocks_1(x2, x1, key_padding_mask)], dim=self.token_dim)
...
if __name__ == "__main__":
    ...
    model = nn.Sequential(*[
                    ResidualCrossAttentionBlock(width, 4)
                    for _ in range(1)])

    # fusion = FusionTransformer(width, 2, 2, fusion="x-attn", pool="pool")
    x = [torch.randn((batch, width)), torch.randn((batch, width))]
    output = model(x[0], x[1])
    print(f'output.shape: {output.shape}')
    # preserve modality 1
    # mask = [torch.ones((batch, 5)).bool(), torch.ones((batch, 4)).bool()]
    # print(fusion(x, mask).shape)
    # print(fusion([x[1]]))
