...
class GNN_graphpredComplete(nn.Module):
    ...
    def forward(self, *argv):
        ...
        node_representation = self.molecule_model(x, edge_index, edge_attr)
        graph_representation = self.pool(node_representation, batch)
        # print(f'node_representation.cpu(): {node_representation.detach().cpu()}')
        return {
            "node_feature": node_representation.detach().cpu(),
            "graph_feature": graph_representation.detach().cpu(),
        }
